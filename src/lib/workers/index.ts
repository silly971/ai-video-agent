...
const workers = isLocalTaskQueue()
  ? []
  : [createImageWorker(), createVideoWorker(), createVoiceWorker(), createTextWorker()]

if (workers.length === 0) {
  console.log('[workers] desktop local queue mode enabled; BullMQ workers are not started')
}
